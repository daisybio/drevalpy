...
process runModels {
    input:
    val model from models_ch

    """
    echo "Running model $model"
    python run_suite.py --model $model --test_mode ${params.test_mode} --randomization_mode ${params.randomization_mode} --randomization_type ${params.randomization_type} --n_trials_robustness ${params.n_trials_robustness} --dataset_name ${params.dataset_name} --path_out ${params.path_out} --curve_curator ${params.curve_curator} --overwrite ${params.overwrite} --optim_metric ${params.optim_metric} --n_cv_splits ${params.n_cv_splits} --response_transformation ${params.response_transformation} --multiprocessing ${params.multiprocessing} 2>&1
    """
}
