...
``FeatureDatasetBlockPredictor`` and ``SingleDrugBlockPredictor`` are
**deprecated** adapters for literature cores that still speak
``FeatureDataset`` after batch assembly. New predictors should not use them;
