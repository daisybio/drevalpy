...
def map_to_cellosaurus(df, cl_dict_ac, cl_dict_sy, species_dict, output_path):
    ...
    for index, row in df.iterrows():
        try:
            ...
            if "Human" not in species:
                warnings.warn(
                    f"Cell line {cl_dict_ac[index]} matched to {index} is not human, but {species}."
                )
        except KeyError:
            try:
                ...
                if "Human" not in species:
                    warnings.warn(
                        f"Cell line {cl_dict_sy[index]} matched to {index} is not human, but {species}."
                    )
                if index not in set_of_unmatched_cell_lines:
                    ...
                    if (
                        SequenceMatcher(
                            a=index.lower(),
                            b=list(cl_dict_ac.keys())[
                                list(cl_dict_ac.values()).index(cl_dict_sy[index])
                            ].lower(),
                        ).ratio()
                        < 0.7
                    ):
    ...
    df = (
        df.reset_index()
        .rename(columns={"index": "cell_line_name"})
        .set_index("cellosaurus_id")
    )
    df.to_csv(output_path)


def map_to_cellosaurus_model_passp(
    df, cl_dict_sid, species_dict, output_path, ignore_columns=[]
):
    # iterate over the cell line names in the dataframe. Try to get the cell line name from the cellosaurus
    # dictionary. If it exists, put it in a column called 'cellosaurus_id'. If it doesn't exist, print the cell line
    # name
    ...
    for cl_name in columns:
        try:
            ...
            if "Human" not in species:
                warnings.warn(
                    f"Cell line {cl_dict_sid[cl_name]} matched to {cl_name} is not human, but {species}."
                )
        except KeyError:
...
def preprocess_methylation():
    ...
    methylation = methylation.rename(index=renamed)
    renamed_cell_lines.update(renamed)
    return methylation
...
def preprocess_mutation():
    ...
    mutation = mutation.rename(index=renamed)
    renamed_cell_lines.update(renamed)
    return mutation
...
def preprocess_gdsc_1():
    ...
    print(
        f"Collapsed {len_before - len(drp)}/{len_before} duplicated ln(IC50) values = {len(drp)} unique values now."
    )
    drp = drp.reset_index()
...
def preprocess_gdsc_1_curves():
    # read in drug response dataframe
    print("Preprocessing drug response dataframe ...")
    drp = pd.read_csv("response_output/GDSC/curves_GDSC1.txt", sep="\t")
    ...
    drp["Name"] = (
        drp["Name"].str.rsplit(" ", n=1).str[0]
        + "|"
        + drp["Name"].str.rsplit(" ", n=1).str[1]
    )
...
def preprocess_gdsc_2_curves():
    # read in drug response dataframe
    print("Preprocessing drug response dataframe ...")
    drp = pd.read_csv("response_output/GDSC/curves_GDSC2.txt", sep="\t")
    # drop all columns except 'Name', 'pEC50', 'Curve Regulation'
    drp = drp[["Name", "pEC50", "Curve Regulation"]]
    # filter out all rows where 'Curve Regulation' is not 'down'
...
def preprocess_sanger_ccle(tpm=True):
    ...
    if tpm:
        gex = pd.read_csv(
            "cell_line_input/SangerCellModelPassports/sanger_tpm_ccle.csv", sep=","
        )
    else:
        gex = pd.read_csv(
            "cell_line_input/SangerCellModelPassports/sanger_counts_ccle.csv", sep=","
        )
    return gex
...
def preprocess_sanger_sanger(tpm=True):
    ...
    if tpm:
        gex = pd.read_csv(
            "cell_line_input/SangerCellModelPassports/sanger_tpm_sanger.csv", sep=","
        )
    else:
        gex = pd.read_csv(
            "cell_line_input/SangerCellModelPassports/sanger_counts_sanger.csv", sep=","
        )
    return gex
...
if __name__ == "__main__":
    ...
    map_to_cellosaurus(
        ...
        cellosaurus_sy_dict,
        species_dict,
        "cell_line_input/GDSC/mutations_cellosaurus.csv",
    )
    ...
    map_to_cellosaurus(
        ...
        cellosaurus_sy_dict,
        species_dict,
        "response_output/GDSC/curves_GDSC1_cellosaurus.csv",
    )
    ...
    map_to_cellosaurus(
        ...
        cellosaurus_sy_dict,
        species_dict,
        "response_output/GDSC/curves_GDSC2_cellosaurus.csv",
    )
    ...
    matched_cell_lines_df = pd.DataFrame.from_dict(
        matched_cell_lines, orient="index", columns=["cellosaurus_id"]
    )
    ...
    renamed_cell_lines_df = pd.DataFrame.from_dict(
        renamed_cell_lines, orient="index", columns=["cell_line_name"]
    )
