...
from drevalpy.resource_management import DeviceManager, memory_management, resource_tracking
...
class DIPKModel(DRPModel):
    ...
    def train(
        ...
        # Use memory management and resource tracking
        with memory_management(monitor_memory=True) as device_manager:
            with resource_tracking():
                ...
                for epoch in range(self.epochs):
                    ...
                    for batch in train_loader:
                        drug_features = batch["molgnet_features"].to(device_manager.device)
                        gene_features = batch["gene_features"].to(device_manager.device)
                        bionic_features = batch["bionic_features"].to(device_manager.device)
                        molgnet_mask = batch["molgnet_mask"].to(device_manager.device)
                        ic50_values = batch["ic50_values"].to(device_manager.device)
                ...
                self.model.load_state_dict(
                    torch.load(checkpoint_path, map_location=device_manager.device, weights_only=True)  # noqa S614
                )
                self.model.to(device_manager.device)  # Ensure model is on the correct device
    ...
    def predict(
        ...
        with torch.no_grad():
            for batch in test_loader:
                drug_features = batch["molgnet_features"].to(self.device_manager.device)
                gene_features = batch["gene_features"].to(self.device_manager.device)
                bionic_features = batch["bionic_features"].to(self.device_manager.device)
                molgnet_mask = batch["molgnet_mask"].to(self.device_manager.device)
