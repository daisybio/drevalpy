...
def load_drug_feature_from_MolGNet(
    ...
    return FeatureDataset(
        features={
            drugs: {
                feature_type: {
                    feature_subtype1: np.array(
                        pd.read_csv(
                            f"{data_path}/{dataset_name}/DIPK_features/Drugs/{drugs}/MolGNet_{drugs}.csv",
                            index_col=0,
                            sep="\t",
                        )
                    ),
                    feature_subtype2: np.array(
                        pd.read_csv(
                            f"{data_path}/{dataset_name}/DIPK_features/Drugs/{drugs}/Edge_Index_{drugs}.csv",
                            index_col=0,
                            sep="\t",
                        )
                    ),
                    feature_subtype3: np.array(
                        pd.read_csv(
                            f"{data_path}/{dataset_name}/DIPK_features/Drugs/{drugs}/Edge_Attr_{drugs}.csv",
                            index_col=0,
                            sep="\t",
                        )
                    ),
...
def GetTestData(cell_id, drug_id, cell_line_features, drug_features):
    ...
    for ii in range(len(Cell)):
        x = torch.tensor(
            drug_features.features[Drug[ii]]["drug_feature_embedding"]["MolGNet_features"], dtype=torch.float32
        )
        edge_index = torch.tensor(
            drug_features.features[Drug[ii]]["drug_feature_embedding"]["Edge_Index"], dtype=torch.float32
        )
        edge_attr = torch.tensor(
            drug_features.features[Drug[ii]]["drug_feature_embedding"]["Edge_Attr"], dtype=torch.float32
        )
        graph = Data(
            x=x,
