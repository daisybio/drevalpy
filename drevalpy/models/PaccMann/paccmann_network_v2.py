"""Contains the PaccMannV2 model for drug response prediction.

The model is based on the PaccMann framework for predicting anticancer drug sensitivity
from SMILES strings and gene expression data.

Original PaccMann repository:
...
class PaccMannV2(nn.Module):
    """PaccMannV2 model for drug response prediction.

    Based on the MCA model in Molecular Pharmaceutics:
        https://pubs.acs.org/doi/10.1021/acs.molpharmaceut.9b00520.

    Main idea:
    - SMILES strings are embedded and processed with convolutional layers
    - gene expression is used as biological context
    - context attention connects gene and drug information
    - the combined representation is passed through dense layers
    - output is a predicted drug sensitivity value
    """

    def __init__(self, params, *args, **kwargs):
        """Initialize the PaccMannV2 model.

        :param params: A dictionary containing the parameter to built the dense encoder.
        :param args: additional positional arguments passed to nn.Module
        :param kwargs: additional keyword arguments passed to nn.Module
        ...
        - activation_fn (string, optional): Activation function used in all layers
        for specification in ACTIVATION_FN_FACTORY. Defaults to 'relu'.
        - batch_norm (bool, optional): Whether batch normalization is applied.
        Defaults to True.
        - dropout (float, optional): Dropout probability in all except parametric layer. Defaults to 0.5.
        - filters (list[int], optional): Numbers of filters to learn per
        SMILES convolutional layer. Defaults to [64, 64, 64].
        - kernel_sizes (list[list[int]], optional): Sizes of kernels per SMILES convolutional layer.
        Defaults to
        ...
        :raises ValueError: if the attention head settings or convolution settings do not match
        """
        ...
        self.molecule_temperature = params.get("molecule_temperature", 1.0)
        self.gene_temperature = params.get("gene_temperature", 1.0)
        ...
        # size of dense input
        self.hidden_sizes = [
            self.molecule_heads[0] * params["smiles_embedding_size"]
            + sum([h * f for h, f in zip(self.molecule_heads[1:], self.filters)])
        ...
        # Convolution layers over embedded SMILES
        self.convolutional_layers = nn.Sequential(
            OrderedDict(
                [
                    ...
                    for index, (num_kernel, kernel_size) in enumerate(zip(self.filters, self.kernel_sizes))
                ]
            )
        )

        # Hidden size of each SMILES representation stage: raw embedding + outputs of conv layer
        smiles_hidden_sizes = [params["smiles_embedding_size"]] + self.filters

        # Attention layers: gene context -> SMILES (focus on relevant molecule parts)
    ...
    def load(self, path, *args, **kwargs):
        ...
        :param args: additional positioinal arguments passed to torch.load
        :param kwargs: additional keyword arguments passed to torch.load
        """
        weights = torch.load(path, *args, **kwargs)  # noqa: S614
    ...
    def save(self, path, *args, **kwargs):
        ...
        :param args: additional positional arguments passed to torch.save
        :param kwargs: additional keyword arguments passed to torch.save
        """
