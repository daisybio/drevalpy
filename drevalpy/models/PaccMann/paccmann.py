...
from .network import PaccMannConfig, PaccMannNetwork
...
def _smiles_column_to_list(smiles_matrix: np.ndarray) -> list[str]:
    """Flatten the single-column SMILES feature matrix into a list of strings.

    :param smiles_matrix: SMILES feature matrix of shape (n_drugs, 1), as returned by load_drug_features
    :return: list of SMILES strings
    """
    return smiles_matrix.reshape(-1).astype(str).tolist()


PAD_IDX = 0
UNK_IDX = 1
BASE_SMILES_VOCAB = {"<pad>": PAD_IDX, "<unk>": UNK_IDX}
...
class PaccMann(DRPModel):
    ...
    and uses the PaccMannNetwork to predict drug response values.
    ...
    - trains a PaccMannNetwork PyTorch model, stopping early once the early stopping set stops improving
    ...
    def __init__(self) -> None:
        ...
        """
        super().__init__()
        self.model: PaccMannNetwork | None = None
        self.hyperparameters = {}

        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.gene_expression_scaler = StandardScaler()

        self.smiles_to_idx: dict[str, int] = dict(BASE_SMILES_VOCAB)
    ...
    def build_model(self, hyperparameters: dict[str, Any]) -> None:
        ...
        The actual PaccMannNetwork is initialized in train(),
    ...
    def _encode_smiles(self, smiles_list: list[str]) -> np.ndarray:
        ...
        for i, smile in enumerate(smiles_list):
            ...
            token_ids = [self.smiles_to_idx.get(token, UNK_IDX) for token in tokens]  # "CCO" -> [2, 2, 3]
    ...
    def _encode_inputs(
        ...
        smiles = _smiles_column_to_list(drug_input.get_feature_matrix("smiles", drug_ids))
    ...
    def _validation_loss(self, validation_loader: DataLoader) -> float:
        ...
        with torch.no_grad():
            for batch_smiles, batch_gex, batch_y in validation_loader:
                ...
                total_loss += functional.mse_loss(predictions, batch_y).item()
    ...
    def train(
        ...
        - train the network, stopping early once the early stopping set has not improved for `patience` epochs
        ...
        :param cell_line_input: FeatureDataset containing cell line features
        :param drug_input: FeatureDataset containing drug features
        :param output_earlystopping: dataset used to select the best epoch and to trigger early stopping.
            If None, the weights of the last epoch are kept and training runs for the full epoch budget.
        ...
        smiles = _smiles_column_to_list(drug_input.get_feature_matrix("smiles", output.drug_ids))
        y = np.asarray(output.response, dtype=np.float32)
        ...
        self.smiles_to_idx = dict(BASE_SMILES_VOCAB)
        ...
        self.model = PaccMannNetwork(PaccMannConfig.from_hyperparameters(model_params)).to(self.device)

        # The dataset holds the drug index rather than the encoded SMILES, so a fresh variant can be drawn for
        # every row in every epoch without materializing one tensor per epoch.
        ...
        for epoch in range(epochs):
            ...
            for batch_drugs, batch_gex, batch_y in train_loader:
                ...
                predictions = self.model(batch_smiles, batch_gex)
                loss = functional.mse_loss(predictions, batch_y)
        ...
        if model_checkpoint_dir is not None:
            torch.save(self.model.state_dict(), f"{model_checkpoint_dir}/paccmann.pt")
    ...
    def predict(
        ...
        # _encode_inputs returns CPU tensors; batches are moved to the device one at a time below.
    ...
    def load(cls, path: str) -> PaccMann:
        ...
        instance.model = PaccMannNetwork(PaccMannConfig.from_hyperparameters(params)).to(instance.device)
