...
from ..utils.layers import ContextAttentionLayer, convolutional_layer, dense_attention_layer, dense_layer
    ...
    def forward(self, smiles, gep, confidence=False):
        ...
        for layer in range(len(self.multiheads)):
            for head in range(self.multiheads[layer]):
                ...
                e, a = self.context_attention_layers[ind](encoded_smiles[layer], context)
                encodings.append(e)
        ...
        if not self.training:
            ...
            smiles_attention_weights = torch.mean(
                torch.cat([torch.unsqueeze(p, -1) for p in smiles_alphas], dim=-1), dim=-1
            )
