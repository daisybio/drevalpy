...
class DRPModel(_DRPLoggingMixin):
    """Experiment-facing drug response model backed by a resolved ``ModelConfig``.

    Subclasses are generated by :func:`~drevalpy.models.construct_model`; do not
    ...
    def __init__(self, hyperparameters: dict[str, Any] | None = None) -> None:
        ...
        Args:
            hyperparameters: Optional flat public hyperparameter overrides. ``None``
                uses the class default config.

        Raises:
            ValueError: If *hyperparameters* cannot be applied to this model class.
        """
    ...
    def model_config(cls) -> ModelConfig:
        ...
        Returns:
            Deep copy of the ``ModelConfig`` bound to this class.

        Raises:
            RuntimeError: If the class was not produced by ``construct_model``.
        """
    ...
    def log_hyperparameters(self, hyperparameters: dict[str, Any]) -> None:
        ...
        Args:
            hyperparameters: Flat public hyperparameters for this instance.
        """
        import wandb
    ...
    def load_cell_line_features(self, data_path: str, dataset_name: str) -> FeatureDataset:
        ...
        Returns:
            Feature views required by the model's cell-line featurizer config.

        Raises:
            RuntimeError: If the model has not been constructed with a ``ModelConfig``.
        """
    ...
    def load_drug_features(self, data_path: str, dataset_name: str) -> FeatureDataset | None:
        ...
        Returns:
            Feature views required by the model's drug featurizer config, or
            ``None`` when the model does not use drug features.
    ...
    def train(
        ...
        Args:
            output: Training response values with cell-line and drug identifiers.
            cell_line_input: Cell-line feature views required by the model config.
            drug_input: Drug feature views, or ``None`` for feature-free predictors.
            output_earlystopping: Optional hold-out responses for early stopping.
            model_checkpoint_dir: Directory for predictor checkpoints during training.

        Raises:
            RuntimeError: If the model lacks a component stack.
        """
    ...
    def predict(
        ...
        Args:
            cell_line_ids: Cell-line identifiers, one per pair.
            drug_ids: Drug identifiers, one per pair.
            cell_line_input: Cell-line feature views required by the model config.
            drug_input: Drug feature views, or ``None`` for feature-free predictors.

        Returns:
            Predicted response values aligned with *cell_line_ids* and *drug_ids*.

        Raises:
            RuntimeError: If the model is untrained or lacks a component stack.
        """
    ...
    def load(cls, path: str | Path) -> DRPModel:
        ...
        Returns:
            Fitted model instance with restored component state.

        Raises:
            FileNotFoundError: If the archive does not exist.
            IncompatibleModelCheckpointError: If the stored model name does not
                match this class.
            CorruptedCheckpointError: If the archive payload is invalid or incomplete.
        """
    ...
    def get_concatenated_features(
        ...
        Args:
            cell_line_view: Cell-line view name, or ``None`` to omit cell-line features.
            drug_view: Drug view name, or ``None`` to omit drug features.
            cell_line_ids_output: Cell-line identifiers for the output pairs.
            drug_ids_output: Drug identifiers for the output pairs.
            cell_line_input: Cell-line feature views, or ``None``.
            drug_input: Drug feature views, or ``None``.

        Returns:
            Feature matrix with one row per output pair.

        Raises:
            ValueError: If a requested view is missing from the inputs.
        """
    ...
    def get_feature_matrices(
        ...
        Returns:
            Mapping from view name to feature matrix aligned with the ids.

        Raises:
            ValueError: If a required view is missing from the inputs.
        """
