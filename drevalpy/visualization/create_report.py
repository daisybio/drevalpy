...
def create_html(run_id, setting):
    ...
    with open(f'../results/{run_id}/{setting}.html', 'w') as f:
        ...
        with open(f'../results/{run_id}/evaluation_results_{setting}.html', 'r') as eval_f:
            ...
            eval_results[0] = eval_results[0].replace('<table border="1" class="dataframe">',
...
def create_index_html(run_id):
    ...
    with open(f'../results/{run_id}/index.html', 'w') as f:
        ...
        settings = [f.split('.html')[0] for f in os.listdir(f'../results/{run_id}') if
                    f.endswith('.html') and f.startswith('L')]
        settings.sort()
...
def draw_violin_and_heatmap(df, run_id, plotname, whole_name=False, normalized_metrics=False):
    ...
    violin.fig.write_html(f'../results/{run_id}/violin_plots/violinplot_{plotname}.html')
...
def draw_scatter_grids_per_group(eval_res_group, group_by, setting, run_id):
    ...
    corr_comp_scatter = CorrelationComparisonScatter(eval_res_group_models, color_by=group_by)
    corr_comp_scatter.dropdown_fig.write_html(f'../results/{run_id}/scatter_eval_models/scatter_eval_models_{group_by}_{setting}.html')
    ...
    for algorithm in eval_res_group_models['algorithm'].unique():
        ...
        corr_comp_scatter = CorrelationComparisonScatter(eval_res_group_algorithm, color_by=group_by)
...
if __name__ == "__main__":
    ...
    for setting in settings:
        ...
        draw_violin_and_heatmap(eval_results_algorithms, run_id, f'algorithms_{setting}_normalized',
        ...
        if setting == 'LPO' or setting == 'LCO':
            ...
            evaluation_results_per_drug_subs.to_html(f'../results/{run_id}/evaluation_results_per_drug_{setting}.html',
                                                     index=False)
            # generate regression plots
            regr_slider = RegressionSliderPlot(df=true_vs_pred_subset, run_id=run_id, group_by='cell_line')
            regr_slider.fig.write_html(
                    f'../results/{run_id}/regression_plots/{setting}_regression_lines_cell_line.html')
            regr_slider_norm = RegressionSliderPlot(df=true_vs_pred_subset, run_id=run_id, group_by='cell_line', normalize=True)
        ...
        if setting == 'LPO' or setting == 'LDO':
            ...
            evaluation_results_per_cell_line_subs = evaluation_results_per_cell_line_subs[
                ['cell_line', 'algorithm', 'rand_setting', 'CV_split',
                 'MSE', 'R^2', 'Pearson', 'RMSE', 'MAE', 'Spearman', 'Kendall', 'Partial_Correlation', 'LPO_LCO_LDO', ]]
            evaluation_results_per_cell_line_subs.to_html(
                f'../results/{run_id}/evaluation_results_per_cell_line_{setting}.html', index=False)
            # generate regression plots
            regr_slider = RegressionSliderPlot(df=true_vs_pred_subset, run_id=run_id, group_by='drug')
            regr_slider.fig.write_html(
                f'../results/{run_id}/regression_plots/{setting}_regression_lines_drug.html')
            regr_slider_norm = RegressionSliderPlot(df=true_vs_pred_subset, run_id=run_id, group_by='drug', normalize=True)
            regr_slider_norm.fig.write_html(
        ...
        eval_results_subset = eval_results_subset[['algorithm', 'rand_setting', 'CV_split',
                                                   ...
                                                   'Spearman: drug normalized', 'Kendall: drug normalized',
                                                   'Partial_Correlation: drug normalized',
                                                   'Spearman: cell_line normalized', 'Kendall: cell_line normalized',
