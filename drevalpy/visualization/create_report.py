...
def pipeline_report():
    """CLI for the pipeline report."""
    parser = argparse.ArgumentParser(description="Make the HTML report for the pipeline.")
    ...
    parser.add_argument(
        "--eval_results_per_drug", type=str, required=True, help="Path to the evaluation results per drug."
    )
    parser.add_argument(
        "--eval_results_per_cl", type=str, required=True, help="Path to the evaluation results per cell line."
    )
    ...
    # make individual html reports per test mode
    generate_reports_for_all_test_modes(
        test_modes=test_modes,
