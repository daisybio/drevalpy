...
class Heatmap:
    def __init__(self, df: pd.DataFrame, normalized_metrics=False, whole_name=False):
        ...
        if self.normalized_metrics:
            ...
            self.df = self.df[[col for col in self.df.columns if 'normalized' not in col]]
    ...
    def __draw_heatmap__(self):
        ...
        self.fig.update_layout(height=1000, width=1100, title_text="Heatmap of the evaluation metrics")
    ...
    def __draw_subplots__(self, plot_setting):
        ...
        if plot_setting == 'standard_errors':
            ...
            corr_columns = [col for col in self.df.columns if
                            'Pearson' in col or 'Spearman' in col or 'Kendall' in col or 'Partial_Correlation' in col]
            corr_columns.sort()
