...
class Violin:
    def __init__(self, df: pd.DataFrame, normalized_metrics=False, whole_name=False):
        ...
        self.fig = go.Figure()
        self.all_metrics = ["R^2", "R^2: drug normalized", "R^2: cell_line normalized",
    ...
    def __draw_violin__(self):
        ...
        count_sum = self.count_r2 + self.count_pearson + self.count_spearman + self.count_kendall + self.count_partial_correlation + self.count_mse + self.count_rmse + self.count_mae
        buttons_update = list([
            ...
            dict(label="R^2",
                 method="update",
                 args=[{"visible": [True] * self.count_r2 + [False] * (count_sum - self.count_r2)},
                       {"title": "R^2"}]),
            dict(label="Correlations",
                 method="update",
                 args=[{"visible": [False] * self.count_r2 + [True] * (
                         self.count_pearson + self.count_spearman + self.count_kendall + self.count_partial_correlation) + [
                                       False] * (
                                           self.count_mse + self.count_rmse + self.count_mae)},
                       {"title": "All Correlations"}]),
            dict(label="Pearson",
                 method="update",
                 args=[{"visible": [False] * self.count_r2 + [True] * self.count_pearson + [False] * (
                         count_sum - self.count_r2 - self.count_pearson)},
                       {"title": "Pearson"},
                       ]),
            dict(label="Spearman",
                 method="update",
                 args=[{"visible": [False] * (self.count_r2 + self.count_pearson) + [True] * self.count_spearman + [
                     False] * (
                                           count_sum - self.count_r2 - self.count_pearson - self.count_spearman)},
                       {"title": "Spearman"}]),
            dict(label="Kendall",
                 method="update",
                 args=[{"visible": [False] * (self.count_r2 + self.count_pearson + self.count_spearman) + [
                     True] * self.count_kendall + [
                                       False] * (
                                           count_sum - self.count_r2 - self.count_pearson - self.count_spearman - self.count_kendall)},
                       {"title": "Kendall"}]),
            dict(label="Partial Correlation",
                 method="update",
                 args=[{"visible": [False] * (
                         count_sum - self.count_partial_correlation - self.count_mse - self.count_rmse - self.count_mae) + [
                                       True] * self.count_partial_correlation + [False] * (
                                           self.count_mse + self.count_rmse + self.count_mae)},
        ...
        if not self.normalized_metrics:
            buttons_update += list([
                dict(label="Errors",
                     method="update",
                     args=[{"visible": [False] * (count_sum - self.count_mse - self.count_rmse - self.count_mae) + [
                         True] * (
                                               self.count_mse + self.count_rmse + self.count_mae)},
                           {"title": "All Errors"}]),
                dict(label="MSE",
                ...
                dict(label="RMSE",
                     method="update",
                     args=[{"visible": [False] * (count_sum - self.count_rmse - self.count_mae) + [
                         True] * self.count_rmse + [
                                           False] * self.count_mae},
                           {"title": "RMSE"}]),
                dict(label="MAE",
                     method="update",
                     args=[{"visible": [False] * (count_sum - self.count_mae) + [True] * self.count_mae},
                           {"title": "MAE"}])
            ])
