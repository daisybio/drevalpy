...
class DrugResponseDataset:
    ...
    def _transformation_kwargs(self, response_transformation: TransformerMixin) -> dict:
        ...
        :returns: {"groups": labels} for group-aware transformations, else {}
        :raises ValueError: if the transformation groups by a field this dataset does not have
        """
        ...
        for field in fields:
            ...
            if values is None:
                raise ValueError(
                    f"{type(response_transformation).__name__} groups the response by '{field}', but the "
                    f"dataset {self.dataset_name} does not provide it. Use a dataset that has a '{field}' "
                    f"column or a response transformation that does not need it."
                )
