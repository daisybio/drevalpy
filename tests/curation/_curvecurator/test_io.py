...
from drevalpy.curation import split
...
def test_curvecurator_to_disk_writes_explicit_output(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    input_file = tmp_path / "Toy_raw.csv"
    input_file.write_text(
        "dose,response,sample,drug\n1.0,0.9,A,D\n10.0,0.1,A,D\n",
        encoding="utf-8",
    ...
    job_id = split_result.work_items[0].work_id
    config_path = job_config_path(output_dir, job_id)
    input_path = job_input_path(output_dir, job_id)
    output_path = tmp_path / "fitted" / "custom_curves.parquet"
    fitted = pd.DataFrame({"Name": ["A|D"], "pEC50": [6.0]})

    monkeypatch.setattr(
        "drevalpy.curation._curvecurator.io._fit_work_item",
        lambda *args, **kwargs: fitted,
    )

    written = curvecurator_to_disk(config_path, input_path, output_path, device="cpu")
