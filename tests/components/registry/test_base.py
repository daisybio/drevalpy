...
def test_set_class_attribute_assigns_value() -> None:
    class Target:
        pass

    set_class_attribute(Target, "registry_name", "demo")
    assert vars(Target)["registry_name"] == "demo"


def test_contract_defined_on_class_detects_body_attribute() -> None:
    class WithContract:
        contract = object()
    ...
    assert contract_defined_on_class(WithContract, "contract")
    assert not contract_defined_on_class(WithoutContract, "contract")
