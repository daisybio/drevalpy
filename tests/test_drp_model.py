...
from drevalpy.models.utils import load_cl_ids_from_csv, load_and_reduce_gene_features, iterate_features, load_drug_ids_from_csv, load_drug_features_from_fingerprints, get_multiomics_feature_dataset, unique
...
def write_gene_list(temp_dir, gene_list):
    ...
    if gene_list == "landmark_genes":
        with open(temp_file, "w") as f:
            f.write("Entrez ID\tSymbol\tName\tGene Family\tType\tRNA-Seq Correlation\tRNA-Seq Correlation Self-Rank\n"
...
@pytest.mark.parametrize("gene_list", [None, "landmark_genes", "drug_target_genes_all_drugs", "gene_list_paccmann_network_prop"])
def test_load_and_reduce_gene_features(gene_list):
    temp = tempfile.TemporaryDirectory()
    os.mkdir(os.path.join(temp.name, "GDSC1_small"))
    temp_file = os.path.join(temp.name, "GDSC1_small", "gene_expression.csv")
    with open(temp_file, "w") as f:
        f.write("CELL_LINE_NAME,TSPAN6,TNMD,BRCA1,SCYL3,HDAC1\n"
    ...
    if gene_list is None:
        ...
        assert np.all(gene_features_gdsc1.meta_info["gene_expression"] == ["TSPAN6", "TNMD", "BRCA1", "SCYL3", "HDAC1"])
...
def test_load_drugs_from_fingerprints():
    ...
    temp_file = os.path.join(temp.name, "GDSC1_small", "drug_fingerprints", "drug_name_to_demorgan_128_map.csv")
    with open(temp_file, "w") as f:
        f.write(
    ...
    assert drug_features_gdsc1.features.keys() == {"Zibotentan", "AZD1208", "CI-1040", "A-83-01", "GSK269962A"}
...
def test_get_multiomics_feature_dataset(gene_list):
    ...
    temp_file = os.path.join(temp.name, "GDSC1_small", "gene_expression.csv")
    with open(temp_file, "w") as f:
        f.write("CELL_LINE_NAME,TSPAN6,BRCA1,DPM1,SCYL3,HDAC1\n"
                "CAL-120,7.632023171463389,2.9645851205892404,10.3795526353077,3.61479404843988,3.38068143582194\n"
                "DMS 114,7.54867116637172,2.77771614989839,11.807341248845802,4.066886747621,3.73248465377029\n"
                "CAL-51,8.71233752103624,2.6435077554121,9.88073281995499,3.95622995046262,3.23662007804984\n"
    ...
    with open(temp_file, "w") as f:
        f.write("CELL_LINE_NAME,chr1:10003165-10003585,chr1:100315420-100316009,chr1:100435297-100436070,chr1:100503482-100504404,chr1:10057121-10058108\n"
    ...
    if gene_list is None:
        assert np.all(dataset.meta_info["gene_expression"] == ["TSPAN6", "BRCA1", "DPM1", "SCYL3", "HDAC1"])
