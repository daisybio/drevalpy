"""Tests for the cell-line featurizer validation shim.

``drevalpy/registry/cell_line_featurizer/_validate.py`` is a pure re-export of the
shared featurizer validation, so the only behaviour to pin is the identity of the
...
from drevalpy.registry.cell_line_featurizer import _validate
