...
def test_gdsc1():
    ...
    assert np.all(
        gdsc1.cell_line_ids[0:3] == np.array(["MC/CAR", "ES3", "ES5"])
    )
    assert np.all(
    ...
    assert np.allclose(
        gdsc1.response[0:3], np.array([2.395685, 3.140923, 3.968757])
    )
...
def test_gdsc2():
    ...
    assert np.allclose(
        gdsc2.response[0:3], np.array([-0.251083, 1.343315, 1.736985])
    )
...
def test_ccle():
    ...
    assert np.all(
        ccle.drug_ids[0:3]
        == np.array(["Tanespimycin", "Saracatinib", "Crizotinib"])
    )
    assert np.allclose(
