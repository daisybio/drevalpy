...
def test_gdsc1():
    ...
    assert np.all(np.array([gdsc1.drug_ids[0], gdsc1.drug_ids[-1]]) == np.array(["Erlotinib", "PFI-3"]))
...
def test_gdsc2():
    ...
    assert np.all(gdsc2.cell_line_ids[0:3] == np.array(["HCC1954", "HCC1143", "HCC1187"]))
    assert np.all(np.array([gdsc2.drug_ids[0], gdsc2.drug_ids[-1]]) == np.array(["Camptothecin", "JQ1"]))
...
def test_ccle():
    ...
    assert np.all(np.array([ccle.cell_line_ids[0], ccle.cell_line_ids[-1]]) == np.array(["U-937", "TEN"]))
    assert np.all(ccle.drug_ids[0:3] == np.array(["Tanespimycin", "Saracatinib", "Crizotinib"]))
    assert np.allclose(ccle.response[0:3], np.array([7.1485739, 8.34797496, 5.75592623]))
